...
def gen_simple_test() -> Dict:
    simple_samplers = "Euler a, DPM++ 2M Karras".split(", ")
    simple_steps = "15, 25".split(", ")
    sample_step_tests = gen_tests_primal(simple_samplers, simple_steps)
    return list_to_test(sample_step_tests)


def gen_minimum_test() -> Dict:
    simple_samplers = ["Euler a"]
    simple_steps = "5, 10".split(", ")
    sample_step_tests = gen_tests_primal(simple_samplers, simple_steps)
    return list_to_test(sample_step_tests)
...
def validate_samplers(sampler_list):
    pass
    # print(sampler_list)
    # valid_samplers = "ddim, euler, euler a, DPM++ 2M Karras".lower().split(", ")
    #
    # for i in sampler_list:
    #     if i.lower() not in valid_samplers:
    #         print("i" +" sampler not found")
    #         raise SamplerNotFoundError
...
def gen_tests(sampler_list:List[str],
    ...
    validate_samplers(sampler_list)
