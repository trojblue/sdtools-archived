...
def mashup_test():
    tags_dict_mashup = {}
    with open("bin/mashup_tags.txt", 'r') as fd:
        lines = fd.readlines()
...
def taboo_test():
    curr_list = ["a", "b", "c", "a c", "a_d"]
    taboo_list = ["c", "a_c", "a d"]
...
def try_gpt():
    ...
    with Image.open("bin/tag_test_img.jpg") as img:
        ...
        # Convert the EXIF data to a UTF-8 encoded string
        exif_data_utf8 = {k: v.decode("utf-8") for k, v in exif_data.items()}

        # Export the EXIF data as a string
