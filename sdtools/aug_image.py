...
from sdtools.fileops import get_files_with_suffix
...
def flip_all_imgs(src_dir, target_dir):
    ...
    image_files = get_files_with_suffix(src_dir, ALL_IMGS)
...
def flip_single_image(src_dir, target_dir, filename):
    ...
    new_name = "flip_" + actual_filename + ".jpg"
...
def create_inverse_trainset(src_dir, target_dir, shuffle=True, filename=True):
    """
    制造读取方向是反向的数据集, 用来弥补容易过拟合的问题
    shuffle: true -> 打乱  | false -> 反序
    filename: true -> 00001_filename.jpg  | false -> 00001_.jpg
    """
    mkdir_if_not_exist(target_dir)
...
def remove_watermark_single(src_dir, target_dir, filename, add_suffix, new_meta):
    ...
    how-do-i-remove-custom-information-in-mecha_tags-png-image-file-that-ive-previously-adde
    :param src_dir:
    :param filename:
    :param target_dir:
    :param add_suffix: 是否在文件里加入_cleaned后缀
...
def clean_routine():
    SOURCE_DIR = "D:\Andrew\Pictures\Grabber\mai.aug\OG"
    TARGET_DIR = "D:\Andrew\Pictures\Grabber\mai.aug"
    backup_dir = "D:\Andrew\Pictures\Grabber\mai.train.dan.1024x2.768x2\OG"

    # purge_info(SOURCE_DIR, TARGET_DIR, backup_dir)

    # 翻转图片
    # do_flip(SOURCE_DIR, TARGET_DIR)

    # 打乱图片
    do_shuffle(SOURCE_DIR, SOURCE_DIR)
