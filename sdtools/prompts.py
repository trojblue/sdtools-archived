...
def remove_mutex(word_list: List[str], mutex_tags: Tuple):  # 02b
    ...
    ***默认输入只有空格, 没有下划线***
    ...
    # mutex_underline = [mecha_tags.replace(" ", "_") for mecha_tags in mutex_space]  # 空格变下划线
    ListA = word_list
...
def __gen_oneline(sdtools, tag_count: int, ) -> List[str]:
    ...
    return gs.start_word + new_tags[:tag_count - end_tags_len] + gs.end_tags  # 返回前n个
...
def get_txt_prompt(sdt, count: int=-1) -> List[str]:
    """
